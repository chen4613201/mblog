...
class LoginForm(FlaskForm):
    loginlabel = StringField(label="用户登录")
...
class RegisterForm(FlaskForm):
    registerlabel = StringField(label="用户注册")
    username = StringField(label=u"用户名", validators=[DataRequired(u"用户名必填！"),length(8,16,"长度为8-16位")])
    password = PasswordField(label=u"密码", validators=[DataRequired(u"密码必填！")])
    password2 = PasswordField(label=u"确认密码", validators=[DataRequired(u"密码必填！"),EqualTo(fieldname="password",message="两次密码不一致！")])
    submit = SubmitField(label="提交")
